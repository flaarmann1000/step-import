    ...
    try:
            ...
            while explorer.More():
                ...
                if triangulation is not None:
                    ...
                    for i in range(1, triangulation.NbTriangles() + 1):
                        ...
                        face_faces.append([
                            3, total_vertex_offset + tri.Value(1) - 1,
                    ...
                    if layer_tool is not None:
                        ...
                        for i in range(1, layers.Length() + 1):
                            ...
                            from OCC.Core.TDataStd import TDataStd_Name_GetID
                            name_attr = layer_label.GetAttribute(TDataStd_Name_GetID())
                            if name_attr is not None:
                                try:
                                    downcast_name_attr = name_attr.GetObject()
                                    if downcast_name_attr is not None:
                                        layer_name = downcast_name_attr.Get().ToExtString()
                                        break
                                except:
                                    pass
        ...
        if all_input_vertices and all_input_faces:
            ...
            for idx, layer in enumerate(unique_layers):
                ...
                layer_faces = [
                    face for j, face in enumerate(all_input_faces)
                    if all_layers[j] == layer
                ]
                if layer_faces:
                    layer_mesh = pv.PolyData(input_vertices,
                                             np.vstack(layer_faces))
                    color = colors[idx][:3]  # Remove alpha channel
                    plotter.add_mesh(layer_mesh,
                                     show_edges=True,
                                     opacity=0.7,
                                     color=color,
                                     label=layer)
            ...
            if len(input_vertices
                   ) >= 3:  # Convex hull requires at least 3 points
                hull = ConvexHull(input_vertices, qhull_options="Qt Qf"
                                  )  # Handle planar surfaces with QJ option
                ...
                for simplex in hull.simplices:
                    ...
                    hull_faces.append(
                        [3] + remapped_simplex)  # Each face has 3 points
                ...
                hull_plotter.add_points(hull_points,
                                        color="blue",
                                        point_size=12,
